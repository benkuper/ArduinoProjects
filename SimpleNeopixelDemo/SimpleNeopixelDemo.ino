...
#define T1L  700    // Width of a 1 bit in ns
...
void theaterChase( unsigned char r , unsigned char g, unsigned char b, unsigned char wait ) {
  ...
  for (int j=0; j< 3 ; j++) {  
    ...
    for (int q=0; q < THEATER_SPACING ; q++) {
      ...
      unsigned int step=THEATER_SPACING;
      ...
      for (int i=0; i < PIXELS ; i++) {
        ...
        if (step<=0) step =THEATER_SPACING;
      }
